"""Widened INDEPENDENT weak labels from OSM vector data -- v5.
...
N = len(rc)
lab = np.lib.format.open_memmap(OUT/'osm_labels5.npy', mode='w+', dtype=np.uint8, shape=(N, CS, CS))
for i, (r, c) in enumerate(rc):
    lab[i] = L[r:r+CS, c:c+CS]
lab.flush()
lp = (lab != 255).reshape(N, -1).mean(1)
np.save(OUT/'osm_label_frac5.npy', lp)
...
json.dump({'aoi_pct':stats,'vacant_km2_landuse':src_lu*px_km2,
          ...
          open('/home/prithvi/AcreVision_v2/outputs/labels5.json','w'), indent=2)
