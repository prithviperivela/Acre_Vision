"""Gold-corrected INDEPENDENT weak labels from OSM vector data -- v7.

v7 change, on top of v6 (scripts/26)
------------------------------------
v6's remaining Vacant error was concentrated in settlements OpenStreetMap has
not building-mapped: 12 of the 46 audited interstitial parcels.  Those places
still have their STREETS mapped, which the building layer does not capture, so
minor-road density is an independent signal that people live there.

Interstitial candidates whose 110 m minor-road density exceeds 0.2554 are dropped
back to `ignore` rather than being called Vacant.  They are not relabelled Urban
-- we know they are not confidently empty, not that they are built.

The threshold was fitted on gold batch 1 by maximising Vacant F1 (batch 1 is
therefore spent as a test of this gate); gold batch 2 is the held-out check.
...
ROAD_GATE = 0.2554          # fitted on gold batch 1, scripts/26_fit_v7_gate.py
...
minor = np.load(OUT/'minor_road_density.npy')      # built by scripts/26
inter = unclaimed & (ring > RING_T) & (bdens < LOCAL_T) & ~corridor
gated = inter & (minor > ROAD_GATE)
print(f'  road gate drops {gated.sum()*px_km2:.1f} km2 of interstitial land back to ignore',
      flush=True)
inter = inter & ~gated
...
N = len(rc)
lab = np.lib.format.open_memmap(OUT/'osm_labels7.npy', mode='w+', dtype=np.uint8, shape=(N, CS, CS))
for i, (r, c) in enumerate(rc):
    lab[i] = L[r:r+CS, c:c+CS]
lab.flush()
lp = (lab != 255).reshape(N, -1).mean(1)
np.save(OUT/'osm_label_frac7.npy', lp)
...
json.dump({'aoi_pct':stats,'vacant_km2_landuse':src_lu*px_km2,
          ...
          open('/home/prithvi/AcreVision_v2/outputs/labels7.json','w'), indent=2)
