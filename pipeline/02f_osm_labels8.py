"""Gold-corrected INDEPENDENT weak labels from OSM vector data -- v8.

v8 change, driven by gold batch 2 (scripts/28)
----------------------------------------------
Batch 2 added a stratum batch 1 lacked: parcels OpenStreetMap does not label at
all.  They are 17,921 of 29,951 valid parcels -- 60% of the city -- and by
verified imagery 22 of the 24 sampled (92%) are genuinely open, developable
land.  No label rule scored so far could see any of them, so the answer key was
not merely imprecise on Vacant, it was blind to most of it.

v6's interstitial rule deliberately required its 810 m surroundings to be
well-mapped (ring > 0.40), to tell "empty" apart from "unsurveyed".  Batch 2
shows that guard is far too conservative for this AOI: the land it excludes is
overwhelmingly vacant anyway.  v8 therefore drops the ring condition, so any
parcel that is unclaimed by every OSM polygon, essentially free of buildings and
not a transport corridor is labelled Vacant.

The remaining risk is unmapped rural woodland becoming false Vacant -- the v3
failure mode.  Gold batch 3 (scripts/30) is sampled from exactly the land this
rule newly claims, and is the test of whether that risk materialises.

(v7's minor-road gate was REJECTED: it improved batch 1, on which it was fitted,
and did not transfer to batch 2.  It is not carried forward.)
...
inter_v6 = unclaimed & (ring > RING_T) & (bdens < LOCAL_T) & ~corridor
inter    = unclaimed & (bdens < LOCAL_T) & ~corridor          # v8: ring condition dropped
print(f'  open land, v6 rule (ring>{RING_T})  +{inter_v6.sum()*px_km2:.1f} km2 '
      f'({inter_v6.mean()*100:.2f}% of AOI)', flush=True)
print(f'  open land, v8 rule (no ring)        +{inter.sum()*px_km2:.1f} km2 '
      f'({inter.mean()*100:.2f}% of AOI)', flush=True)
np.save(OUT/'v8_new_open.npy', (inter & ~inter_v6))           # what v8 newly claims
...
N = len(rc)
lab = np.lib.format.open_memmap(OUT/'osm_labels8.npy', mode='w+', dtype=np.uint8, shape=(N, CS, CS))
for i, (r, c) in enumerate(rc):
    lab[i] = L[r:r+CS, c:c+CS]
lab.flush()
lp = (lab != 255).reshape(N, -1).mean(1)
np.save(OUT/'osm_label_frac8.npy', lp)
...
json.dump({'aoi_pct':stats,'vacant_km2_landuse':src_lu*px_km2,
          ...
          open('/home/prithvi/AcreVision_v2/outputs/labels8.json','w'), indent=2)
